...
const priorityStyles = {
  High: { bar: 'bg-red-500', badge: 'bg-red-50 text-red-600 ring-1 ring-red-100' },
  Medium: { bar: 'bg-amber-500', badge: 'bg-amber-50 text-amber-700 ring-1 ring-amber-100' },
  Low: { bar: 'bg-emerald-500', badge: 'bg-emerald-50 text-emerald-700 ring-1 ring-emerald-100' }
}
...
function isOverdue(dateStr) {
  if (props.variant === 'completed' || !dateStr) return false
...
<template>
  ...
  >
    <div class="w-14 h-14 rounded-2xl bg-[var(--color-muted)] grid place-items-center mx-auto mb-4">
      <AppIcon :name="variant === 'completed' ? 'check-circle' : 'inbox'" class="w-7 h-7 text-[var(--color-primary)]" />
    </div>
    <h3 class="text-sm font-semibold text-[var(--color-ink)]">
      {{ variant === 'completed' ? 'No completed tasks yet' : 'No ongoing tasks' }}
    </h3>
    <p class="text-sm text-[var(--color-muted-fg)] mt-1 max-w-xs mx-auto">
      {{
        variant === 'completed'
          ? 'Tasks you mark as complete will show up here.'
          : 'Try a different search term, or add a new task to get started.'
      }}
    </p>
  </div>
  ...
  <ul v-else class="space-y-3">
    <li
      ...
      :class="variant === 'completed' ? 'opacity-80 hover:opacity-100' : ''"
    >
      <div class="flex">
        <span
          class="w-1.5 shrink-0"
          :class="variant === 'completed' ? 'bg-emerald-500' : priorityStyle(task.priority).bar"
          aria-hidden="true"
        ></span>

        <div class="flex-1 min-w-0 px-4 sm:px-5 py-4 flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4">
          <div class="flex-1 min-w-0">
            <div class="flex flex-wrap items-center gap-x-2 gap-y-1">
              <h3
                class="font-semibold text-[var(--color-ink)] truncate"
                :class="variant === 'completed' ? 'line-through decoration-2 decoration-[var(--color-muted-fg)]/50' : ''"
              >
                {{ task.title }}
              </h3>
              <span
              ...
              <span v-else class="text-xs font-medium px-2 py-0.5 rounded-full" :class="priorityStyle(task.priority).badge">
                {{ task.priority }}
              </span>
            </div>
            <div class="flex flex-wrap items-center gap-x-3 gap-y-1 mt-1.5 text-xs text-[var(--color-muted-fg)]">
              <span class="inline-flex items-center gap-1">
                <AppIcon name="clipboard" class="w-3.5 h-3.5" />
                {{ task.subject }}
              </span>
              <span
                class="inline-flex items-center gap-1"
                :class="isOverdue(task.dueDate) ? 'text-red-600 font-medium' : ''"
              >
                <AppIcon name="calendar" class="w-3.5 h-3.5" />
                {{ formatDate(task.dueDate) }}
                <span v-if="isOverdue(task.dueDate)">&middot; overdue</span>
              </span>
            </div>
          </div>
          ...
          <div class="flex items-center gap-2 shrink-0 self-start sm:self-center">
            ...
            >
              <AppIcon :name="variant === 'completed' ? 'restore' : 'check-circle'" class="w-4 h-4" />
              {{ variant === 'completed' ? 'Restore' : 'Complete' }}
            </button>
